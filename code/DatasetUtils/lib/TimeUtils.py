...
def date_range(start, end, step=1, format="%Y_%m_%d"):
    ...
    date_range_list = [strftime(strptime(start, format) + datetime.timedelta(i), format) for i in range(0, days, step)]
    ...
    for index, image_path in enumerate(src_images):
        ...
        if gt_timestamp == timestamp:
            for num in range(interval):
                interval_index = min(max(index - int(interval/2) + num, 0), len(src_images) - 1)
