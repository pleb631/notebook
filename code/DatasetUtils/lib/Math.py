...
class Distance:
    ...
    def calcu_iou(self, box1, box2, mode='iou'):
        ...
        if mode == 'diou':
            ...
            c_center_len = self.calcu_euclidean_distance(get_xyxy_center(box1), get_xyxy_center(box2))
            c_diagonal_len = self.calcu_euclidean_distance([c_xmin, c_ymin], [c_xmax, c_ymax])
    ...
    def calcu_boxes_iou_matrix(self, boxes, iou_mode='iou', mode='diagonal', extend_params=None):
        """计算多个box之间的iou矩阵
        ...
        for i in range(len(boxes)):
            if mode == 'diagonal':
                ...
                bbox = [max(min(bbox_xyxy[0], width), 0), max(min(bbox_xyxy[1], height), 0), 
                                max(min(bbox_xyxy[2], width), 0), max(min(bbox_xyxy[3], height), 0)]
    ...
    def calcu_boxes_ab_distance(self, boxes_a, boxes_b):
        ...
        boxes_a_xywh = np.zeros_like(boxes_a)
        boxes_a_xywh[:, 0] = (boxes_a[:, 0] + boxes_a[:, 2]) / 2
        boxes_a_xywh[:, 1] = (boxes_a[:, 1] + boxes_a[:, 3]) / 2
        boxes_a_xywh[:, 2] = boxes_a[:, 2] - boxes_a[:, 0]
        boxes_a_xywh[:, 3] = boxes_a[:, 3] - boxes_a[:, 1]

        boxes_b_xywh = np.zeros_like(boxes_b)
        boxes_b_xywh[:, 0] = (boxes_b[:, 0] + boxes_b[:, 2]) / 2
        boxes_b_xywh[:, 1] = (boxes_b[:, 1] + boxes_b[:, 3]) / 2
        boxes_b_xywh[:, 2] = boxes_b[:, 2] - boxes_b[:, 0]
        boxes_b_xywh[:, 3] = boxes_b[:, 3] - boxes_b[:, 1]

        distance_x_matrix = np.fabs(np.repeat(boxes_a_xywh[:, :1], len(boxes_b), axis=1) - np.repeat(boxes_b_xywh[:, :1].T, len(boxes_a), axis=0))/np.repeat(boxes_a_xywh[:, 2:3]/2, len(boxes_b), axis=1)
        # x出界
        distance_x_matrix_out_range = distance_x_matrix>1
        distance_x_matrix *= np.repeat(boxes_a_xywh[:, 2:3]/2, len(boxes_b), axis=1)

        distance_y_matrix =-(np.repeat(boxes_a[:, 1:2]+boxes_a_xywh[:, 3:4]*y_ratio, len(boxes_b), axis=1) - np.repeat(boxes_b_xywh[:, 1:2].T, len(boxes_a), axis=0))/np.repeat(boxes_a_xywh[:, 3:4]/2, len(boxes_b), axis=1)
        # y出界
        distance_y_matrix_out_range = (distance_y_matrix>(2*(1-y_ratio))) | (distance_y_matrix<(-2*y_ratio))
...
def rotated_rectangle(center, size, angle):
    ...
    # 利用宽高构建一个以原点为中心的矩形框
    origin_rect = [(0-int(width/2), 0-int(height/2)), (0+int(width/2), 0-int(height/2)), (0+int(width/2), 0+int(height/2)), (0-int(width/2), 0+int(height/2))]
...
def others_numpy():
    ...
    a = a[::5]
    # 生成等间隔数组
    internal_index = np.linspace(0, len(tracking_quality_seq)-1, keyframe_num, endpoint=True, dtype='int64')
...
def calcu_batch_tensor_angle_torch(tensor_a, tensor_b, mode='radian'):
    ...
    tensor_ab_angle_radian_clamp = torch.clamp(tensor_ab_cos_angle, -1+epsilon, 1-epsilon)
    ...
    if mode == 'radian':
        ...
        # # 只支持pytorch >= 1.6.0
        tensor_ab_angle_degree = torch.rad2deg(tensor_ab_angle_radian)
        # tensor_ab_angle_degree = torch.Tensor(math.degrees(tensor_ab_angle_radian))
        return tensor_ab_angle_degree
...
class Trigonometric:
    ...
    def compute_point_angle(P1, P2, Q1, Q2):
        ...
        angle = Trigonometric.compute_vector_angle(x, y, minangle=True)
        return angle
