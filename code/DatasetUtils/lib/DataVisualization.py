...
def draw_box_with_text(image, box, text, color_idx, fontScale=1, fontThickness=2):
    ...
    colors = [
        ...
        (61, 219, 134), (26, 147, 52), (0, 212, 187), (44, 153, 168), (0, 194, 255), (52, 69, 147), 
        (100, 115, 255), (0, 24, 236), (255, 56, 56), (255, 157, 151), (255, 112, 31), (255, 178, 29), 
    ...
    cv2.putText(image, text, (c1[0], c1[1] - 2), 0, fontScale, txt_color, thickness=fontThickness, lineType=cv2.LINE_AA)
...
class BasicDataVis:
    ...
    def draw_boxes_ab_with_text(self, boxes_a, boxes_b, ab_relation):
        ...
        for box_a_idx, box_a in enumerate(boxes_a):
            draw_box_with_text(image, boxes_a[box_a_idx], f'{box_a_idx}', box_a_idx%20, 0.5, 1)
        ...
        for box_b_idx, box_b in enumerate(boxes_b):
            ...
            for relation_a_idx, relation_b_idx, relation in ab_relation:
                if box_b_idx == relation_b_idx:
                    text += f'{relation_a_idx}:{relation:.3f};'
...
class DetDataVis(BasicDataVis):
    ...
    def show_yolo_txt(self, root, ratio, classes, mode='show', mode2='std'):
        ...
        rand_list = random.sample(range(0, len(txt_files)), int(len(txt_files)*ratio))
        for index, txt_file in enumerate(txt_files):
            if index not in rand_list:
                continue
            print(index, txt_file)
            image = cv2.imread(os.path.join(yolo_path.replace('Annotations', 'JPEGImages'), txt_file.replace('.txt', '.jpg')))
            width = image.shape[1]
            height = image.shape[0]
            txt_data = read_yolo_txt(os.path.join(yolo_path, txt_file), width, height, mode2)

            for box in txt_data:
                cv2.rectangle(image, (int(box[0]), int(box[1])), (int(box[2]),int(box[3])), (0, 255, 0), 3, lineType=cv2.LINE_AA)
                cv2.putText(image, classes[box[4]], (int(box[0]), int(box[1])), cv2.FONT_HERSHEY_COMPLEX,1,(0,255,0),1)
                if len(box) == 7 and (box[5] != -1) and (box[6] != -1):
                    cv2.circle(image, (int(box[5]),int(box[6])), radius = 5, color = (0,255,0), thickness=-1)
                    cv2.line(image, (int(box[5]),int(box[6])), (int((box[0]+box[2])/2), box[3]), color=(0,255,0), thickness=3, lineType=cv2.LINE_AA)
            
            if mode == 'show':
                cv2.imshow('show', image)
                cv2.waitKey(0)
            elif mode == 'write':
                os.makedirs(os.path.join(yolo_path.replace('Annotations', 'show_result_txt')), exist_ok=True)
                cv2.imwrite(os.path.join(yolo_path.replace('Annotations', 'show_result_txt'), txt_file.replace('.txt', '.jpg')),  image)
    ...
    def show_voc_xml(self, root, ratio, mode='show'):
        ...
        rand_list = random.sample(range(0, len(xml_files)), int(len(xml_files)*ratio))
        for idx, xml_file in enumerate(xml_files):
            if idx not in rand_list:
                continue
            print(idx, xml_file)
            xml_data = read_xml(os.path.join(root, 'Annotations_XML', xml_file), 'std')
            image = cv2.imread(os.path.join(root, 'JPEGImages', xml_file.replace('.xml', '.jpg')))

            for box in xml_data['bndboxes']:
                cv2.rectangle(image, (int(box[0]), int(box[1])), (int(box[2]),int(box[3])), (0, 255, 0), 2)
                cv2.putText(image, box[4], (int(box[0]), int(box[1])), cv2.FONT_HERSHEY_COMPLEX,1,(0,255,0),1)
            ...
            if mode == 'show':
                ...
                cv2.imwrite(os.path.join(root, 'show_result_xml', xml_file.replace('.xml', '.jpg')), image)

    def show_prelabel(self, prelabel_json_path, img_dir_path, save_img_dir_path, ratio, mode='show'):
        ...
        rand_list = random.sample(range(0, len(prelabel_json_data)), int(len(prelabel_json_data)*ratio))
        for idx, image_info in enumerate(prelabel_json_data):
            if idx not in rand_list:
                continue
            print(idx, len(prelabel_json_data))
            image_path = os.path.join(img_dir_path, os.path.basename(image_info['url_image']))
            ...
            if mode == 'show':
                ...
                cv2.imwrite(os.path.join(save_img_dir_path, os.path.basename(image_info['url_image'])), image)
    ...
    def badcase_det_box_vis(self, image, correct_boxes, incorrect_boxes, missed_boxes):
        ...
        for correct_box in correct_boxes:
            cv2.rectangle(image, (int(correct_box[0]), int(correct_box[1])), (int(correct_box[2]),int(correct_box[3])), (0, 255, 0), 2)

        for incorrect_box in incorrect_boxes:
            cv2.rectangle(image, (int(incorrect_box[0]), int(incorrect_box[1])), (int(incorrect_box[2]),int(incorrect_box[3])), (0, 0, 255), 2)

        for missed_box in missed_boxes:
            cv2.rectangle(image, (int(missed_box[0]), int(missed_box[1])), (int(missed_box[2]),int(missed_box[3])), (255, 0, 0), 2)
    ...
    def badcase_det_box_hip_vis(self, image, gt_box_hip, pred_box_hip):
        ...
        '''
        # gt
        cv2.rectangle(image, (int(gt_box_hip[0]), int(gt_box_hip[1])), (int(gt_box_hip[2]),int(gt_box_hip[3])), (0, 255, 0), 3)
        cv2.circle(image, (int(gt_box_hip[5]),int(gt_box_hip[6])), radius =5, color=(0, 255, 0), thickness=-1)
        cv2.line(image, (int(gt_box_hip[5]),int(gt_box_hip[6])), (int((gt_box_hip[0]+gt_box_hip[2])/2), int(gt_box_hip[3])), color=(0, 255, 0), thickness=3, lineType=cv2.LINE_AA)

        # pred
        cv2.rectangle(image, (int(pred_box_hip[0]), int(pred_box_hip[1])), (int(pred_box_hip[2]),int(pred_box_hip[3])), (0, 0, 255), 3)
        if pred_box_hip[6] != -1:
            cv2.circle(image, (int(pred_box_hip[6]),int(pred_box_hip[7])), radius =5, color=(0, 0, 255), thickness=-1)
            cv2.line(image, (int(pred_box_hip[6]),int(pred_box_hip[7])), (int((pred_box_hip[0]+pred_box_hip[2])/2), int(pred_box_hip[3])), color=(0, 0, 255), thickness=3, lineType=cv2.LINE_AA)

        person_box = head_shoulder_box2person_box(gt_box_hip[:4], 4.0, 1080, 1920)
        person_image = crop_image_extend_border(image, person_box, 0.4, 0.4)
...
class DetKeypointDataVis(BasicDataVis):
    ...
    def show_voc_xml(self, mode='show'):
        ...
        for index, xml_file in enumerate(xml_files):
            ...
            xml_data = read_xml(os.path.join(self.root, 'Annotations_XML', xml_file), 'with_hip_mid_keypoint')
            image = cv2.imread(os.path.join(self.root, 'JPEGImages', xml_file.replace('.xml', '.jpg')))
            ...
            if mode == 'show':
                ...
                cv2.imwrite(os.path.join(self.root, 'show_result_xml', xml_file.replace('.xml', '.jpg')), image)
...
class KeypointDataVis(BasicDataVis):
    ...
    def draw_bag_keypoint_using_txt(self, txt_path):
        ...
        for index,line in enumerate(txt_data):
            ...
            # image = cv2.imread(image_path)
            image = cv2.imread(os.path.join(self.root, '女包高清大图_v10', image_path.split('/')[-1][9:]))
            ...
            show_image_path = os.path.join(self.save_dir_path, os.path.basename(image_path))
