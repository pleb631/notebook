...
class BadcaseAnalyseUtils:
    ...
    def match_boxes_iou(self, boxes_a, boxes_b, iou_mode="iou", iou_threshold=0.5):
        ...
        iou_list = self.distance.calcu_boxes_ab_iou(
            boxes_a_xy, boxes_b_xy, iou_mode, "list"
        )
        ...
        for elem in iou_list:
            ...
            if (box_a_idx in idxes_a) and (box_b_idx in idxes_b) and iou > iou_threshold:
                matched_boxes.append(
                    boxes_a[box_a_idx] + boxes_b[box_b_idx] + [iou]
                )
                idxes_a.remove(box_a_idx)
                idxes_b.remove(box_b_idx)
    ...
    def analyse_false_miss_box(self, gt_boxes, pred_boxes, iou_threshold=0.5):
        ...
        if gt_len > 0:
            for index in range(nd):
                ...
                inters = iw * ih
                uni = (
                    (pre_box[2] - pre_box[0] + 1.0) * (pre_box[3] - pre_box[1] + 1.0)
                    + (gt_boxes_array[:, 2] - gt_boxes_array[:, 0] + 1.0)
                    * (gt_boxes_array[:, 3] - gt_boxes_array[:, 1] + 1.0)
                    - inters
                )
                ...
                if ovmax >= iou_threshold and flag_gt[jmax] < 1:
                    correct_boxes.append(
                        pre_box.tolist() + gt_boxes_array[jmax, :].tolist()
                    )
                    flag_gt[jmax] = 1
                else:
                    incorrect_boxes.append(pre_box.tolist())
            missed_boxes.extend(
                gt_boxes_array[id, :] for id in range(gt_len) if flag_gt[id] < 1
            )
        return correct_boxes, incorrect_boxes, missed_boxes

    def save_false_miss_det_image(
        self, pred_json_path, save_dir_path, crop_bad_box_flag=False
    ):
        ...
        for index, image_path in enumerate(json_data.keys()):
            ...
            assert os.path.exists(image_path), f"image_path: {image_path} is not exists! "
            ...
            correct_boxes, false_boxes, missed_boxes = self.analyse_false_miss_box(
                gt_boxes, pred_boxes_list
            )
            ...
            if len(false_boxes) != 0 or len(missed_boxes) != 0:
                ...
                show_image = self.vis_utils.badcase_det_box_vis(
                    image.copy(), correct_boxes, false_boxes, missed_boxes
                )
                cv2.imwrite(
                    os.path.join(save_dir_path, "draw_box", imageName), show_image
                )
                ...
                if len(false_boxes) != 0:
                    for idx, false_box in enumerate(false_boxes):
                        if float(false_box[4]) > 0.5:
                            continue
                        false_box_img = crop_image_extend_border(
                            image, false_box[:4], 0.0, 0.0
                        )
                        cv2.imwrite(
                            os.path.join(
                                save_dir_path, "false_box", f"{idx}_{imageName}"
                            ),
                            false_box_img,
                        )
                if len(missed_boxes) != 0:
                    for idx, missed_box in enumerate(missed_boxes):
                        missed_box_img = crop_image_extend_border(
                            image, missed_box, 0.0, 0.0
                        )
                        cv2.imwrite(
                            os.path.join(
                                save_dir_path, "miss_box", f"{idx}_{imageName}"
                            ),
                            missed_box_img,
                        )
...
class ReIDBadcaseUtils(BadcaseAnalyseUtils):
    ...
    def gen_query_topk(
        self,
        gallery_features_npy_path,
        query_features_npy_path,
        gallery_label_ids_npy_path,
        query_label_ids_npy_path,
        gallery_images_ids_json_path,
        query_images_ids_json_path,
        save_json_path,
        k=5,
    ):
        ...
        for i in range(num_q):
            ...
            tmp_query_topk_images = [query_image_path]
