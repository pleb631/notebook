...
def human_seg_track_fuse(track_cfd, dl_cfd, dl_weights, is_track):
    """光流追踪图和人像分割结构融合
        光流跟踪图是由上一帧的预测分割图通过光流warp到当前帧的结果图,
        假设上一帧的预测分割图是高质量图,则warp后的也是高质量图,
        但对于运动的部分可能存疑,
        所以在融合的时候,在光流能跟踪的部分，高度采信光流跟踪图,并且在绝对确信的地方(绝对是前景部分或绝对是背景部分且光流能跟踪到的部分),
        对不动点按0.7采信光流追踪图,动点0.6采信光流来进行松弛(存在绝对是前景部分但光流未跟踪到的可能性)
    输入参数:
        track_cfd: 光流追踪图
        dl_cfd: 当前帧分割结果
        dl_weights: 融合权重图
